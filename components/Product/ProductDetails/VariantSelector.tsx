        ...
        {product?.variants?.map((item, index) => (
          <div
            ...
            className={
              ...
              (currentVariant === String(index + 1) ? " border-green-500" : "")
            }
            key={index}
          >
      ...
      {variant?.color && (
        <div className="flex flex-col gap-2 mb-3">
          ...
          <div className="flex items-center text-black font-light gap-2 select-none cursor-pointer">
            <button className="bg-neutral-100 border px-2 rounded-full hover:bg-neutral-200 hover:border-neutral-400 transition-all ease-linear">
              Black
            </button>
            <button className="bg-neutral-100 border px-2 rounded-full hover:bg-neutral-200 hover:border-neutral-400 transition-all ease-linear">
              Blue
          ...
          <div className="flex items-center text-black font-light gap-2 select-none cursor-pointer">
            {variant.options.map((option, index) => (
              <button
                ...
                className={
                  "flex items-center justify-center min-w-10 w-12 bg-neutral-100 border px-2 rounded-full enabled:hover:scale-105 disabled:cursor-not-allowed disabled:ring-1 disabled:ring-neutral-300 disabled:text-neutral-400 disabled:before:absolute disabled:before:h-px disabled:before:w-[36px] disabled:before:-rotate-45 disabled:before:bg-neutral-400 transition-all ease-linear" +
                  (option.option == currentSize
                    ? " border-green-600 bg-green-300/40 enabled:hover:border-green-700"
                    : "")
                }
              >
                {option.option}
